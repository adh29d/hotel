...
A guest scans a QR code on the back of their door, lands on
`/room/[roomNumber]`, sees their balance, optionally extends checkout and
orders a coffee, pays in one go, and leaves a review.

This is a demo only — there is no backend, no real payment processing, and
no real RMS integration. Everything is wired to local mock data so the full
flow feels real on a phone.
...
Open http://localhost:3000 — you'll be redirected to `/room/204`, the room
the QR code would resolve to in the demo. Step through:

`Landing → Pay & checkout → Confirmation → Review`
...
- React Context for in-memory order state across pages (refresh resets — by
  design)

## Where things live

- `lib/mockData.ts` — single source of truth: the demo reservation, coffee
  menu, late-checkout pricing config, pickup location. Edit this to tweak
  prices/copy.
- `lib/OrderContext.tsx` — order state + computed totals.
- `app/page.tsx` — redirects to the demo room.
- `app/room/[roomNumber]/page.tsx` — landing (greeting, balance, slider,
  coffee entry).
- `app/room/[roomNumber]/payment/page.tsx` — Apple Pay / Samsung Pay / card.
- `app/room/[roomNumber]/confirmation/page.tsx` — success + receipt + review.
- `components/` — `LateCheckoutSlider`, `CoffeeOrder`, `BottomSheet`,
  `QuantityStepper`, `AnimatedTotal`, `SuccessTick`, `ReviewCard`.
...
| Coffee order | Local menu + fake pickup time | Kitchen ticketing system at Pelicans Breakfast Restaurant on payment success |
